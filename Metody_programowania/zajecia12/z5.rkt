...
(define-type Exp
  ...
  (lamE [x : Symbol] [e : Exp])
  (appE [e1 : Exp] [e2 : Exp])
  (recE [name : Symbol] [x : Symbol] [e : Exp])) ; 
...
(module+ test
  ...
  (test (parse `{rec fact {n} {if {= n 0}
                                  1
                                  {* n {fact {- n 1}}}}})
        (recE 'fact 'n (ifE (appE (appE (varE '=) (varE 'n)) (numE 0))
                            (numE 1) 
                            (appE    
                             (appE (varE '*) (varE 'n))
                             (appE
...
(define-type Value
  ...
  (recV [name : Symbol] [x : Symbol] [e : Exp] [env : Env]) 
...
(define (apply [v1 : Value] [v2 : Value]) : Value
  (type-case Value v1
    ...
    [(recV name x exp env) ;
     (eval exp (extend-env (extend-env env name v1) x v2))] 
    [(primopV f) ; 
     (f v2)]    
  ...
  (test (run `{let a 1
                {let p {lambda {x} {+ x a}}
                  {let a 10
                    {p 0}}}})
        (numV 1))
  (test (run `{{rec fact {n} {if {= n 0} 
                                 1       
                                 {* n {fact {- n 1}}}}}
               5})
        (numV 120))
  (test (run `{{rec Fib {n} {if {= n 0}    
                                0         
                                {if {= n 1} 
                                    {+ {Fib {- n 1}} {Fib {- n 2}}}}}}
               10})
